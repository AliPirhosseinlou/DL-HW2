...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        out=torch.matmul(x_reshaped, w) + b   # Linear transformation
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        out, relu_cache = ReLU.forward(fc_out)  # ReLU activation
...
def softmax_loss(x, y):
    ...
    ######################################################################
    # Replace "pass" statement with your code
    scores = torch.exp(x - torch.max(x, dim=1, keepdim=True).values)
    softmax_probs = scores / torch.sum(scores, dim=1, keepdim=True)
    
    N = x.shape[0]
    loss = -torch.sum(torch.log(softmax_probs[range(N), y])) / N
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx, grads['W1'], grads['b1'] = Linear_ReLU.backward(dh1, cache_h1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            layer_input, cache = Linear_ReLU.forward(layer_input, self.params[f'W{i}'], self.params[f'b{i}'])
            if self.use_dropout:
                layer_input, drop_cache = Dropout.forward(layer_input, self.dropout_param)
                relu_caches.append(drop_cache)
            affine_caches.append(cache)

        scores, last_cache = Linear.forward(layer_input, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        for i in range(1, self.num_layers + 1):
            W = self.params[f'W{i}']
            regularization_loss += 0.5 * self.reg * torch.sum(W ** 2)

        loss += regularization_loss

        # Backpropagation
        dx, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dscores, last_cache)
        grads[f'W{self.num_layers}'] += self.reg * self.params[f'W{self.num_layers}']

        for i in reversed(range(1, self.num_layers)):
            if self.use_dropout:
                dx = Dropout.backward(dx, relu_caches.pop())

            dx, dw, db = Linear_ReLU.backward(dx, affine_caches[i - 1])
